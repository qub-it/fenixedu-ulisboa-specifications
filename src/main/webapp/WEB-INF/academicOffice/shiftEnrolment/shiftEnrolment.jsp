	...
	app.controller("ShiftEnrolmentCtrl", [
			...
			function($scope, $http) {
				...
				$scope.fetchPossibleShiftsToEnrol = function(registrationID, executionCourseID, shiftType) {
					...
					$http.get(window.contextPath + '/academicAdministration/shiftEnrolment/possibleShiftsToEnrol.json/' + registrationID + '/' + executionCourseID + '/' + shiftType).success(
							function(result) {
...
<c:forEach items="${enrolmentBeans}" var="enrolmentBean" varStatus="index">
	<c:if test="${index.first}"><c:set var="registration" value="${enrolmentBean.registration}" /></c:if>
</c:forEach>

<h1><spring:message code="label.title.shiftEnrolment"/> 
		<small><c:out value="${registration.student.person.name} (${registration.student.number})" /></small>
</h1>

<c:set var="req" value="${pageContext.request}" />
<a href="${fr:checksumLink(req, '/academicAdministration/manageStudentCurricularPlans.do?method=list&registrationId='.concat(registration.externalId))}">
...
<div ng-app="shiftEnrolmentApp">
	...
	<c:if test="${not empty enrolmentBeans}">
		...
		<ul class="nav nav-tabs">
			<c:forEach items="${enrolmentBeans}" var="enrolmentBean">
				<li role="presentation" class="<c:out value="${enrolmentBean.selected ? 'active' : ' '}" />">
					<a href="${pageContext.request.contextPath}/academicAdministration/shiftEnrolment/${enrolmentBean.registration.externalId}/${enrolmentBean.executionSemester.externalId}">
						<c:out value="${enrolmentBean.executionSemester.qualifiedName}" /> 
		...
		<c:set var="schoolClass" value="${selectedEnrolmentBean.schoolClass}" />		
		<p class="text-muted mtop15"><spring:message code="label.registrationSchoolClass"/>: <span class="bold"><c:out value="${not empty schoolClass ? schoolClass.editablePartOfName : ' - '}" /></span><p>
							...
							<table class="table table-bordered" ng-show="shiftsToEnrol.length > 0">
								<tbody>
									<tr ng-repeat="shift in shiftsToEnrol track by $index">
										<td>{{shift.lessons}}</td>
										<td>
											<a href="${pageContext.request.contextPath}/academicAdministration/shiftEnrolment/addShift/${selectedEnrolmentBean.registration.externalId}/${selectedEnrolmentBean.executionSemester.externalId}/{{shift.externalId}}">
			...
			<div id="removeShiftModal" class="modal fade bs-remove-shift-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-sm" >
					<div class="modal-content">
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-default" data-dismiss="modal"><spring:message code="button.cancel"/></button>	
							<a class="btn btn-primary" href="${pageContext.request.contextPath}/academicAdministration/shiftEnrolment/removeShift/${selectedEnrolmentBean.registration.externalId}/${selectedEnrolmentBean.executionSemester.externalId}/{{shiftID}}" role="button"><spring:message code="button.confirm"/></a>
						</div>
					</div>
				</div>
			</div>			
