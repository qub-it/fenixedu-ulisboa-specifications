...
<section>
				...
				<tbody>
					...
					<c:if test="${not empty professorships}">
						...
						<tr data-professorship="${professorship.externalId}" data-responsible="${professorship.responsibleFor}">
							<td>
								<%= org.fenixedu.ulisboa.specifications.domain.services.ExecutionCourseServices.getCode(((org.fenixedu.academic.domain.Professorship) pageContext.getAttribute("professorship")).getExecutionCourse()) %>
								 - <c:out value="${professorship.executionCourse.nameI18N.content}" />
							</td>
							<td>
	...
	<form:form id="search" role="form" modelAttribute="bean" action="${baseUrl}/${authorization.externalId}" method="GET" class="form-horizontal">
		...
		<div class="form-group">
			...
			<div class="col-sm-11">
				<form:select path="degree" id="selectDegree" class="form-control" disabled="${empty degrees}">
				    <c:forEach var="degree" items="${degrees}">
				        <form:option value="${degree.externalId}"><c:out value="${degree.degree.code} - ${degree.presentationName}"/></form:option>
				    </c:forEach>
				</form:select>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-11">
				<form:select path="course" id="selectCourse" itemLabel="nameI18N.content" class="form-control" disabled="${empty courses}">
				    <c:forEach var="course" items="${courses}">
				        <form:option value="${course.externalId}">
					        <%= org.fenixedu.ulisboa.specifications.domain.services.ExecutionCourseServices.getCode(((org.fenixedu.academic.domain.ExecutionCourse) pageContext.getAttribute("course"))) %>
					        <c:out value=" - ${course.nameI18N.content}"/>
				        </form:option>
				    </c:forEach>
			    </form:select>								
			</div>
