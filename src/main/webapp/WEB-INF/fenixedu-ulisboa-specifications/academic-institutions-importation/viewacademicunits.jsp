...
<div class="well well-sm" style="display:inline-block">
	...
	<a class="" href="${pageContext.request.contextPath}<%= AcademicInstitutionsImportationController.VIEW_DEGREE_DESIGNATIONS_URL %>">
		<spring:message code="label.AcademicInstitutionsImportationController.viewdegreedesignations.title"/>
	</a>
	
	&nbsp;|&nbsp;
	<span class="glyphicon glyphicon-upload" aria-hidden="true"></span>&nbsp;
	<a onclick="javascript:processUpload();" href="#">
		<spring:message code="label.AcademicInstitutionsImportationController.upload.academic.units.title"/>
	</a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="uploadForm" method="POST" enctype="multipart/form-data"
            	action="${pageContext.request.contextPath}<%=AcademicInstitutionsImportationController.UPLOAD_ACADEMIC_UNITS_FILE_URL%>" >
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    <h4 class="modal-title">
                        <spring:message code="label.AcademicInstitutionsImportationController.upload.file.official.academic.units.title" />
                    </h4>
                </div>
	...
	<tbody>
		<c:forEach var="unit" items="${officialAcademicUnits}">
			<tr>
				...
				<td>
					<a class="btn btn-default btn-xs" href="${pageContext.request.contextPath}<%= AcademicInstitutionsImportationController.VIEW_ACADEMIC_UNIT_DETAIL_URL %>/${unit.externalId}">
...
<h2><spring:message code="label.AcademicInstitutionImportationController.unofficial.academic.units" /></h2>

<table class="table" id="unofficialAcademicUnitsTable" >
	<thead>
		<tr>
			<th scope="row" class="col-xs-3"><spring:message code="label.AcademicInstitutionsImportationController.academicUnit.name" /></th>
			<th scope="row" class="col-xs-3"><spring:message code="label.AcademicInstitutionsImportationController.degreeDesignations.number" /></th>
	...
	<tbody>
		<c:forEach var="unit" items="${unofficialAcademicUnits}">
			<tr>
				<td><c:out value="${unit.nameI18n.content}" /></td>
				<td><c:out value="${unit.degreeDesignationSet.size()}" /></td>
				<td style="width: 10px;">
...
$(document).ready(function() {
	...
	dataTables(
			"unofficialAcademicUnitsTable",
			true,
			true,
			true,
			"${pageContext.request.contextPath}",
			"${datatablesI18NUrl}",
			[ { "width": "5%", "targets": 1 },  { "width": "5%", "targets": 2 } ]);
