...
<div class="page-header">
    <h1>
        <spring:message code="label.firstTimeCandidacy.DownloadDocuments" />
        <small></small>
    </h1>
</div>
...
angular.module('angularApp', ['ngSanitize', 'ui.select', 'bennuToolkit']).controller('angularController', ['$scope', function($scope) {
    ...
    $scope.printDocuments = function () {
        ...
        $('form').attr('action',beginUrl + $scope.object.registration + endUrl);
        $('form').submit();
    };
...
<form name='form' target="_blank" method="post" class="form-horizontal"
    ng-app="angularApp" ng-controller="angularController" action="#">
    ...
    <div class="well">
        ...
        <ul>
            <li><spring:message
                    code="label.firstTimeCandidacy.finished.details.registrationDeclaration" /></li>
            <li><spring:message
                    code="label.firstTimeCandidacy.finished.details.model43" /></li>
            <li><spring:message
                    code="label.firstTimeCandidacy.finished.details.tuitionPayments" /></li>
        </ul>
    </div>

    <div class="form-group row">
        <div class="col-sm-2 control-label required-field">
            <spring:message code="label.student.registration" />
        </div>

        <div class="col-sm-10">
            <ui-select id="student_registration" name="registration"
                ng-model="$parent.object.registration" ng-required="true"
                theme="bootstrap"> <ui-select-match>{{$select.selected.text}}</ui-select-match>
            <ui-select-choices
                repeat="registration.id as registration in object.registrationValues | filter: $select.search">
            <span ng-bind-html="registration.text"></span> </ui-select-choices> </ui-select>
        </div>
    </div>

    <%-- NAVIGATION --%>
        <button id="downloadATag" class="btn btn-primary" ng-disabled="form.$invalid" ng-click="printDocuments()" type="button" role="button">
        <span class="glyphicon glyphicon-ok" aria-hidden="true"></span>&nbsp;
            <label><spring:message code="label.event.firstTimeCandidacy.download" /></label>
        </button>
</form>
