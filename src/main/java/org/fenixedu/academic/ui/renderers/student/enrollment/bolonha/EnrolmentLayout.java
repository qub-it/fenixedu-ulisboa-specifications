...
import org.fenixedu.academic.domain.curricularRules.ConditionedRoute;
...
public class EnrolmentLayout extends BolonhaStudentEnrolmentLayout {
	...
	// qubExtension
	static private ExecutionInterval getExecutionInterval(final StudentCurricularPlan input,
	...
	protected void generateGroup(final HtmlBlockContainer blockContainer,
		...
		// init
		setEmptyGroup(studentCurriculumGroupBean,
				!studentCurriculumGroupBean.getCurriculumModule().isBranchCurriculumGroup());

		// qubExtension, don't GENERATE concluded groups
		if (canPerformStudentEnrolments || !isConcluded(studentCurriculumGroupBean.getCurriculumModule(),
	...
	@Override
	protected void addGroupHeaderRow(final HtmlTable groupTable,
			final StudentCurriculumGroupBean studentCurriculumGroupBean, final ExecutionSemester executionSemester) {
		...
		if (studentCurriculumGroupBean.getCurriculumModule().isRoot()) {
			...
			titleCell.setBody(new HtmlText(
					buildCurriculumGroupLabel(studentCurriculumGroupBean.getCurriculumModule(), executionSemester),
					false));
		}
		...
		MetaObject enrolmentMetaObject = MetaObjectFactory
				.createObject(studentCurriculumGroupBean.getCurriculumModule(), new Schema(CurriculumGroup.class));
	...
	@Override
	protected String buildCurriculumGroupLabel(final CurriculumGroup curriculumGroup,
	...
	static public void addCreditsDistributionMessage(final CurriculumGroup group, final ExecutionSemester semester,
		...
		final Double concluded = CurriculumModuleServices.getCreditsConcluded(group, semester.getExecutionYear())
				.doubleValue();

		if (approved.doubleValue() != concluded.doubleValue()) {
			result.append(" <span class=\"wrongCreditsDistributionError\" title=\"");
			result.append(i18n(Bundle.APPLICATION, "label.curriculumGroup.wrongCreditsDistribution",
	...
	static public boolean hasMinimumCredits(final CurriculumGroup group, final ExecutionSemester semester) {
		final CreditsLimit creditsRule = (CreditsLimit) group
				.getMostRecentActiveCurricularRule(CurricularRuleType.CREDITS_LIMIT, semester);
		return creditsRule != null && creditsRule.getMinimumCredits().doubleValue() > 0d
				&& CurriculumModuleServices.getCreditsConcluded(group, semester.getExecutionYear())
	...
	// qubExtension, more credits info
	static public void addEnroledEcts(final CurriculumGroup group, final ExecutionSemester semester,
			final StringBuilder result) {
		final ExecutionInterval interval = getExecutionInterval(group.getStudentCurricularPlan(), semester);

		result.append(", <span title=\"");
		result.append(i18n(Bundle.APPLICATION, "label.curriculum.credits.legend.enroledCredits",
	...
	// qubExtension, more credits info
	static public void addSumEcts(final CurriculumGroup group, final ExecutionSemester semester,
			final StringBuilder result) {
		final ExecutionInterval interval = getExecutionInterval(group.getStudentCurricularPlan(), semester);
		...
		result.append(", <span title=\"");
		result.append(
				i18n(Bundle.APPLICATION, "label.curriculum.credits.legend.totalCredits", interval.getQualifiedName()));
	...
	private boolean isConcluded(final CurriculumGroup group, final ExecutionYear year) {
		final CreditsLimit creditsRule = (CreditsLimit) group
				.getMostRecentActiveCurricularRule(CurricularRuleType.CREDITS_LIMIT, year);
	...
	protected void generateCurricularCoursesToEnrol(final HtmlTable groupTable,
		...
		final List<IDegreeModuleToEvaluate> coursesToEvaluate = filterCurricularCoursesToEvaluate(
				studentCurriculumGroupBean);
		...
		for (final IDegreeModuleToEvaluate degreeModuleToEvaluate : coursesToEvaluate) {
			...
			String degreeModuleName = degreeModuleToEvaluate.getDegreeModule().getNameI18N(executionSemester)
					.getContent();

			if (degreeModuleToEvaluate.getDegreeModule().isLeaf()
					&& !degreeModuleToEvaluate.isOptionalCurricularCourse()) {
				...
				if (canPerformStudentEnrolments) {
					...
					if (gradeScaleChain != GradeScale.TYPE20) {
						degreeModuleName += " (" + i18n(Bundle.STUDENT, "label.grade.scale") + " - "
			...
			aggregationCell.setBody(generateAggregationInfo(degreeModuleToEvaluate.getContext(),
					getStudentCurricularPlan(), executionSemester));
			...
			if (!degreeModuleToEvaluate.isOptionalCurricularCourse()) {
				...
				actionLink.setName(
						"optionalCurricularCourseLink" + degreeModuleToEvaluate.getCurriculumGroup().getExternalId()
	...
	private boolean filterByCompetenceCourse(final CurricularCourse curricularCourse) {
		return FenixEduAcademicExtensionsConfiguration.getConfiguration()
				.getCurricularRulesApprovalsAwareOfCompetenceCourse()
		...
		return input.isToBeDisabled()
				...
				|| isStudentLogged() &&
						...
						(appliesAnyRules(input.getCurriculumModule(), true /* recursive */,
								...
								|| appliesAnyRules(input.getCurriculumModule(), false /* recursive */,
	...
	final private boolean appliesAnyRules(final IDegreeModuleToEvaluate input,
		...
		if (input != null && input.getContext() != null && curricularRuleClasses != null) {
			...
			for (final Class<? extends CurricularRule> curricularRuleClass : curricularRuleClasses) {
				...
				List<? extends ICurricularRule> rules = CurricularRuleServices.getCurricularRules(
						input.getDegreeModule(), parentCourseGroup, curricularRuleClass, executionInterval);
				...
				rules = CurricularRuleServices.getCurricularRules(parentCourseGroup, curricularRuleClass,
						executionInterval);
				if (!rules.isEmpty() && rules.iterator().next().appliesToContext(input.getContext())) {
					return true;
				}
	...
	@Override
	protected void generateEnrolments(final StudentCurriculumGroupBean studentCurriculumGroupBean,
	...
	@Override
	protected void generateEnrolment(final HtmlTable groupTable, final Enrolment enrolment,
			final String enrolmentNameClasses, final String enrolmentYearClasses, final String enrolmentSemesterClasses,
			final String enrolmentEctsClasses, final String enrolmentCheckBoxClasses) {

		final EnroledCurriculumModuleWrapper wrapper = new EnroledCurriculumModuleWrapper(enrolment,
	...
	@Override
	protected void generateGroups(final HtmlBlockContainer container,
			final List<IDegreeModuleToEvaluate> degreeModulesToSelect, final StudentCurriculumGroupBean bean,
			final StudentCurricularPlan plan, final ExecutionSemester executionSemester, final int depth) {

		// first enroled
		final List<StudentCurriculumGroupBean> enroledGroups = bean
	...
	static private String getAggregatorEntryStyle(final CurriculumAggregatorEntry entry,
			final StudentCurricularPlan scp, final ExecutionInterval interval) {
