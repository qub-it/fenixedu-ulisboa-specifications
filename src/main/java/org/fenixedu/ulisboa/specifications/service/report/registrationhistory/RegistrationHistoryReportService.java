...
public class RegistrationHistoryReportService {
	...
	private Set<ProgramConclusion> calculateProgramConclusions() {
		...
		final Set<DegreeType> degreeTypesToProcess = this.degreeTypes.isEmpty()
				? DegreeType.all().collect(Collectors.toSet()) : this.degreeTypes;
	...
	private Predicate<RegistrationHistoryReport> filterPredicate() {
		...
		final Predicate<RegistrationHistoryReport> protocolFilter = r -> this.registrationProtocols
				.contains(r.getRegistrationProtocol());
		if (!this.registrationProtocols.isEmpty()) {
			result = result.and(protocolFilter);
		}

		final Predicate<RegistrationHistoryReport> ingressionTypeFilter = r -> this.ingressionTypes
				.contains(r.getIngressionType());
		...
		final Predicate<RegistrationHistoryReport> firstTimeFilter = r -> (this.firstTimeOnly && r.isFirstTime())
				|| (!this.firstTimeOnly && !r.isFirstTime());
		...
		final Predicate<RegistrationHistoryReport> statuteTypeFilter = r -> r.getStudentStatutes().stream()
	...
	private Set<Registration> buildSearchUniverse(final ExecutionYear executionYear) {
		...
		if (withEnrolments) {
			...
			result.addAll(stream.map(enrolment -> enrolment.getRegistration()).filter(studentNumberFilter).filter(
					registration -> RegistrationDataServices.getRegistrationData(registration, executionYear) != null)
					.collect(Collectors.toSet()));

		} else if (this.firstTimeOnly != null && this.firstTimeOnly) {
			result.addAll(
					executionYear.getStudentsSet().stream().filter(studentNumberFilter).collect(Collectors.toSet()));
		} else if (this.registrationStateTypes != null && !this.registrationStateTypes.isEmpty()) {
			...
			result.addAll(Bennu.getInstance().getRegistrationStatesSet().stream()
					...
					.filter(s -> filterByRegistrationStateType(s, executionYear))
	...
	private boolean filterByRegistrationStateType(final RegistrationState s, final ExecutionYear executionYear) {
		if(isRegistrationStateActive(s)) {
			return s.getExecutionYear().isBeforeOrEquals(executionYear);
		} else if(isRegistrationStateConcluded(s)) {
			return isRegistrationConcludedInExecutionYear(s.getRegistration(), executionYear);
		} else {
			return s.getExecutionYear() == executionYear;
		}
	}
	...
	static protected BigDecimal calculateExecutionYearWeightedAverage(final RegistrationHistoryReport report) {
		...
		for (final Enrolment enrolment : enrolmentsByYear.stream().filter(normalEnrolmentFilter(report))
			...
			gradesSum = gradesSum
					.add(enrolment.getGrade().getNumericValue().multiply(enrolment.getEctsCreditsForCurriculum()));
			creditsSum = creditsSum.add(enrolment.getEctsCreditsForCurriculum());
		}
		;

		return gradesSum.compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ZERO
				: gradesSum.divide(creditsSum, MathContext.DECIMAL128).setScale(3, RoundingMode.HALF_UP);
	}
	...
	static protected BigDecimal calculateExecutionYearSimpleAverage(final RegistrationHistoryReport report) {
		...
		return gradesSum.compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ZERO
				: gradesSum.divide(BigDecimal.valueOf(total), MathContext.DECIMAL128).setScale(3, RoundingMode.HALF_UP);
	}
	...
	// TODO: refactor to use RegistrationConclusionServices.inferConclusion =>
	// refactor method to allow return non conclued
	static protected void addConclusion(final RegistrationHistoryReport report) {
		...
		for (final StudentCurricularPlan studentCurricularPlan : report.getRegistration()
				.getStudentCurricularPlansSet()) {
			for (final ProgramConclusion programConclusion : ProgramConclusion.conclusionsFor(studentCurricularPlan)
					.collect(Collectors.toSet())) {
				conclusions.put(programConclusion,
				...
				if (conclusionsByProgramConclusion.size() == 1) {
					...
					report.addConclusion(iter,
							conclusionsByProgramConclusion.stream()
									.sorted(RegistrationConclusionServices.CONCLUSION_BEAN_COMPARATOR.reversed())
									.findFirst().get());
				}
			}
		}
