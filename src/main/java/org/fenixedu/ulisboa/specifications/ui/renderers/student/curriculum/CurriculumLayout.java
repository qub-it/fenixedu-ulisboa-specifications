...
public class CurriculumLayout extends Layout {
	...
	private void generateAverageTableHeader(final HtmlTable table) {
		...
		generateCellWithText(row, ULisboaSpecificationsUtil.bundle("label.gradingTables.curriculumRenderer.ectsGrade"),
				classes).setStyle("width: 100px");
		generateCellWithText(row, BundleUtil.getString(Bundle.APPLICATION, "label.grade"), classes)
				.setStyle("width: 35px");
		generateCellWithText(row, BundleUtil.getString(Bundle.APPLICATION, "label.weight"), classes)
				.setStyle("width: 35px");
		generateCellWithText(row, ULisboaSpecificationsUtil.bundle("label.approvalType"), classes)
				.setStyle("width: 75px");
		generateCellWithText(row, ULisboaSpecificationsUtil.bundle("label.approvalInfo"), classes)
				.setStyle("width: 120px");
		generateCellWithText(row, ULisboaSpecificationsUtil.bundle("label.creditsInfo"), classes)
				.setStyle("width: 120px");
		generateCellWithText(row, ULisboaSpecificationsUtil.bundle("label.approvementDate"), classes)
				.setStyle("width: 80px");
		;
	}
	...
	private void generateAverageRow(final HtmlTable mainTable, final AverageEntry averageEntry) {
		...
		StudentCurricularPlanLayout.generateDate(averageEntry.getConclusionDateOnTarget(), row, (String) null,
				(String) null);
	}
	...
	private void generateEctsGradeCell(HtmlTableRow enrolmentRow, final ICurriculumEntry entry) {
		...
		if (entry instanceof ExternalEnrolment) {
			...
			if (line.getCurricularCourse() == null) {
				ectsGrade = ULisboaSpecificationsUtil
						.bundle("label.gradingTables.curriculumRenderer.lineWithoutCurricularCourse");
			} else {
	...
	private void generateWeightCell(HtmlTableRow enrolmentRow, final ICurriculumEntry entry) {
		generateCellWithText(enrolmentRow,
				entry.getGrade().isNumeric() ? entry.getWeigthForCurriculum().toString() : "-",
				renderer.getWeightCellClass());
	}

	static private HtmlTableCell generateCellWithText(final HtmlTableRow row, final String text,
			final String cssClass) {
		return StudentCurricularPlanLayout.generateCellWithText(row, text, cssClass);
	}

	private void generateCellWithLink(final HtmlTableRow row, final ExecutionYear executionYear,
