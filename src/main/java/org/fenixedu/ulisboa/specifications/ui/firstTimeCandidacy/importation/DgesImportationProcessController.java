...
@SpringFunctionality(app = FenixeduUlisboaSpecificationsController.class, title = "link.dgesStudentImportationProcess", accessGroup = "academic(MANAGE_CANDIDACY_PROCESSES)")
...
public class DgesImportationProcessController extends FenixeduUlisboaSpecificationsBaseController {
	...
	private void setDgesImportProcessConfigurationBean(final DgesImportProcessConfigurationBean bean,
			final Model model) {
		if (bean != null) {
			bean.updateLists();
		}
	...
	public String search(@RequestParam(value = "executionYear", required = false) ExecutionYear executionYear,
		...
		List<DgesStudentImportationProcess> importationJobsDone = DgesStudentImportationProcess
				.readDoneJobs(executionYear).stream()
				.sorted(DgesStudentImportationProcess.COMPARATOR_BY_BEGIN_DATE.reversed()).collect(Collectors.toList());
		List<DgesStudentImportationProcess> importationJobsPending = DgesStudentImportationProcess
				.readUndoneJobs(executionYear).stream()
				.sorted(DgesStudentImportationProcess.COMPARATOR_BY_BEGIN_DATE.reversed()).collect(Collectors.toList());
	...
	public String create(@RequestParam(value = "bean", required = true) final DgesImportProcessBean bean,
			@RequestParam(value = "importationFile", required = false) final MultipartFile importationFile,
		...
		try {
			...
			if (bean.getExecutionYear() != null) {
				redirectAttributes.addFlashAttribute("selectedExecutionYearId",
						bean.getExecutionYear().getExternalId());
			}
	...
	public @ResponseBody String createpostback(
			@RequestParam(value = "bean", required = true) final DgesImportProcessBean bean, final Model model) {
		setDgesBaseProcessBean(bean, model);
	...
	public String cancel(@PathVariable("oid") final DgesStudentImportationProcess process, final Model model,
		...
		try {
			...
			addInfoMessage(BundleUtil.getString(ULisboaConstants.BUNDLE,
					"message.DgesStudentImportationProcess.cancelled.with.success"), model);
	...
	public String resend(@PathVariable("oid") final DgesStudentImportationProcess process, final Model model,
		...
		try {
			...
			addInfoMessage(BundleUtil.getString(ULisboaConstants.BUNDLE,
					"message.DgesStudentImportationProcess.resended.with.success"), model);
		} catch (DomainException ex) {
			addErrorMessage(BundleUtil.getString(ULisboaConstants.BUNDLE, ex.getKey()), model);
		}
	...
	private void resendImportationProcessJob(final DgesStudentImportationProcess process) {
		...
		if (process.getIsNotDoneAndNotCancelled()) {
			throw new ULisboaSpecificationsDomainException(
					"error.DgesStudentImportationProcess.resend.not.cancelled.job", process.getDescription());
		}
		...
		try {
			if (process.getDone() != null && process.getDone()) {
				...
				String filename = URLEncoder.encode(
						StringNormalizer.normalizePreservingCapitalizedLetters(process.getFile().getDisplayName())
								.replaceAll("\\s", "_"),
						"UTF-8");
	...
	public void downloadCGDTemplate(@PathVariable("oid") final CgdMod43Template template,
			final HttpServletResponse response) {
		try {

			response.setContentType(template.getContentType());
			String filename = URLEncoder.encode(StringNormalizer
					.normalizePreservingCapitalizedLetters(template.getDisplayName()).replaceAll("\\s", "_"), "UTF-8");
			response.setHeader("Content-disposition", "attachment; filename=" + filename);
	...
	public String readConfiguration(final Model model) {
		...
		final Predicate<Degree> isDegreeActive = d -> d.getExecutionDegrees().stream()
				.anyMatch(ed -> ed.getExecutionYear().isCurrent());

		FirstYearRegistrationGlobalConfiguration globalConfiguration = FirstYearRegistrationGlobalConfiguration
				.getInstance();
		List<FirstYearRegistrationConfiguration> activeDegreesMappings = globalConfiguration
				.getFirstYearRegistrationConfigurationsSet().stream().filter(c -> isDegreeActive.test(c.getDegree()))
	...
	public String updateConfiguration(
			...
			@RequestParam(value = "cgdTemplateFile", required = true) final MultipartFile cgdTemplateFile,
			final Model model, final RedirectAttributes redirectAttributes) {
	...
	@RequestMapping(value = _MANAGE_CONTINGENT_MAPPING_URI, method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	...
	@RequestMapping(value = _MANAGE_DEGREE_CONFIGURATION_URI, method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	...
	private void createDegreeConfiguration(final Degree degree, final DegreeCurricularPlan degreeCurricularPlan,
		...
		if (FirstYearRegistrationConfiguration.getDegreeConfiguration(degree) != null) {
			FirstYearRegistrationConfiguration.getDegreeConfiguration(degree).edit(degreeCurricularPlan,
					requiresVaccination);
		} else {
	...
	@RequestMapping(value = _DELETE_DEGREE_CONFIGURATION_URI, method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
