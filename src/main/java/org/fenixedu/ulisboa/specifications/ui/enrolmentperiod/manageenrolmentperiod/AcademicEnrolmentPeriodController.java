...
public class AcademicEnrolmentPeriodController extends FenixeduUlisboaSpecificationsBaseController {
    ...
    public String search(@RequestParam(value = "executionSemester", required = false) ExecutionSemester executionSemester,
            ...
            Model model) {
        model.addAttribute("executionSemesters", ExecutionSemester.readNotClosedExecutionPeriods());
    ...
    private static final String _ADD_CURRICULAR_PLAN_URI = "/add/curricularplan";
    public static final String ADD_CURRICULAR_PLAN_URL = CONTROLLER_URL + _ADD_CURRICULAR_PLAN_URI;

    @RequestMapping(value = _ADD_CURRICULAR_PLAN_URI + "/{academicEnrolmentPeriodId}/{degreeCurricularPlanId}",
    ...
    public @ResponseBody String addDegreeCurricularPlan(
            ...
            @RequestParam(value = "bean", required = true) AcademicEnrolmentPeriodBean bean, Model model) {
        if (!academicEnrolmentPeriod.getDegreeCurricularPlansSet().contains(degreeCurricularPlan)) {
            addDegreeCurricularPlan(academicEnrolmentPeriod, degreeCurricularPlan);
        }
        bean.setDegreeCurricularPlans(
                academicEnrolmentPeriod.getDegreeCurricularPlansSet().stream().collect(Collectors.toList()));
        setAcademicEnrolmentPeriodBean(bean, model);
        return getBeanJson(bean);
    }

    @Atomic
    public void addDegreeCurricularPlan(AcademicEnrolmentPeriod academicEnrolmentPeriod,
    ...
    private static final String _ADD_STATUTE_TYPE_URI = "/add/statutetype";
    public static final String ADD_STATUTE_TYPE_URL = CONTROLLER_URL + _ADD_STATUTE_TYPE_URI;

    @RequestMapping(value = _ADD_STATUTE_TYPE_URI + "/{academicEnrolmentPeriodId}/{statuteTypeId}", method = RequestMethod.POST,
            produces = "application/json;charset=UTF-8")
    public @ResponseBody String addStatuteType(
            @PathVariable("academicEnrolmentPeriodId") AcademicEnrolmentPeriod academicEnrolmentPeriod,
            @PathVariable("statuteTypeId") StatuteType statuteType,
            @RequestParam(value = "bean", required = true) AcademicEnrolmentPeriodBean bean, Model model) {
        if (!academicEnrolmentPeriod.getStatuteTypesSet().contains(statuteType)) {
            addStatuteType(academicEnrolmentPeriod, statuteType);
        }
        bean.setStatuteTypes(academicEnrolmentPeriod.getStatuteTypesSet().stream().collect(Collectors.toList()));
        setAcademicEnrolmentPeriodBean(bean, model);
        return getBeanJson(bean);
    }

    @Atomic
    public void addStatuteType(AcademicEnrolmentPeriod academicEnrolmentPeriod, StatuteType statuteType) {
