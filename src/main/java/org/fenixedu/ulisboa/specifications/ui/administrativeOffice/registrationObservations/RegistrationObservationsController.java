...
public class RegistrationObservationsController extends StrutsFunctionalityController {
	...
	public String home(@PathVariable("registration") Registration registration, Model model,
...
//        }
	...
	private boolean getWriteAccessControl(Registration registration) {
		...
		boolean academicAuthorization = AcademicAuthorizationGroup
				.get(AcademicOperationType.STUDENT_ENROLMENTS, registration.getDegree()).isMember(user)
				|| PermissionService.hasAccess("STUDENT_ENROLMENTS", registration.getDegree(), user);
	...
	public String prepareEdit(
			@PathVariable("registrationObservations") RegistrationObservations registrationObservations, Model model,
			HttpServletRequest request) {
